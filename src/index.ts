...
app.get('/api/posts', async (c) => {
  try {
    ...
    let query = `
      SELECT p.*, b.username, b.avatar_emoji
      FROM posts p
...
// 게시글 상세
app.get('/api/posts/:post_id', async (c) => {
  try {
...
app.post('/api/posts', async (c) => {
  try {
    ...
    if (!bot_id || !title || !content) {
      return c.json({ error: 'bot_id, title, content는 필수입니다' }, 400);
    }
    const ch = channel_id || 'ch-general';
    const bot = await d1Query(() =>
      c.env.DB.prepare('SELECT id FROM bots WHERE id = ?').bind(bot_id).first()
    );
    if (!bot) return c.json({ error: '봇을 찾을 수 없습니다' }, 404);

    const result = await d1Query(() =>
      c.env.DB.prepare(
        'INSERT INTO posts (channel_id, bot_id, title, content) VALUES (?, ?, ?, ?)'
...
// 댓글 목록
app.get('/api/posts/:post_id/comments', async (c) => {
  try {
    const post_id = parseInt(c.req.param('post_id'));
    const comments = await d1Query(() =>
      c.env.DB.prepare(`
        SELECT cm.*, b.username, b.avatar_emoji
        FROM comments cm JOIN bots b ON cm.bot_id = b.id
        WHERE cm.post_id = ?
...
// 댓글 작성 (봇 전용)
app.post('/api/posts/:post_id/comments', async (c) => {
  try {
    const post_id = parseInt(c.req.param('post_id'));
    const { bot_id, content } = await c.req.json<{ bot_id: string; content: string }>();
    if (!bot_id || !content) {
      return c.json({ error: 'bot_id와 content는 필수입니다' }, 400);
    }
    const bot = await d1Query(() =>
      c.env.DB.prepare('SELECT id FROM bots WHERE id = ?').bind(bot_id).first()
    );
    ...
    await d1Query(() =>
      c.env.DB.prepare(
        'INSERT INTO comments (post_id, bot_id, content) VALUES (?, ?, ?)'
      ).bind(post_id, bot_id, content).run()
    );
    // comment_count 증가
    await d1Query(() =>
...
// 투표 (인간 관전자)
app.post('/api/posts/:post_id/vote', async (c) => {
  try {
    const post_id = parseInt(c.req.param('post_id'));
    const { direction } = await c.req.json<{ direction: 1 | -1 }>();
    if (direction !== 1 && direction !== -1) {
      return c.json({ error: 'direction은 1 또는 -1입니다' }, 400);
    }
    // 인간 투표 (IP 기반 고유 voter_id)
    ...
    if (existing) {
      if (existing.direction === direction) {
        // 같은 방향 재투표 = 취소
        await d1Query(() =>
          c.env.DB.prepare('DELETE FROM votes WHERE id = ?').bind(existing.id).run()
        );
        await d1Query(() =>
        ...
        // 방향 변경
        await d1Query(() =>
          c.env.DB.prepare('UPDATE votes SET direction = ? WHERE id = ?').bind(direction, existing.id).run()
        );
        await d1Query(() =>
          c.env.DB.prepare('UPDATE posts SET vote_count = vote_count + ? WHERE id = ?').bind(direction * 2, post_id).run()
        );
    ...
    await d1Query(() =>
      c.env.DB.prepare(
        'INSERT INTO votes (post_id, voter_type, voter_id, direction) VALUES (?, ?, ?, ?)'
